...
class Bids(DirTree):
    ...
    def put_fileset_paths(self, fileset: FileSet, fs_paths: ty.Iterable[Path]):

        stored_paths = super().put_fileset_paths(fileset, fs_paths)
        for fs_path in stored_paths:
            if fs_path.suffix == ".json":
                # Ensure TaskName field is present in the JSON side-car if task
                # is in the filename
                self._edit_json(fileset, fs_path)
        return stored_paths

    def _edit_json(self, fileset: FileSet, fs_path: str):
